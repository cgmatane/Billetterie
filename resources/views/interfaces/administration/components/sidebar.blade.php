<div class="nav-side-menu">
    <div class="menu-list">

        <ul id="menu-content" class="menu-content collapse out">
            <li>
                <a href="#">
                    <i class="fas fa-eye"></i> Vue générale
                </a>
            </li>

            <li  data-toggle="collapse" data-target="#destination" class="collapsed active">
                <a href="#"><i class="fas fa-map-pin"></i> Destination <span class="arrow"></span></a>
            </li>
            <ul class="sub-menu collapse" id="destination">
                <li><a href="#">Ajouter</a></li>
                <li><a href="#">Supprimer</a></li>
                <li><a href="#">Editer</a></li>
            </ul>

            <li data-toggle="collapse" data-target="#trajet" class="collapsed">
                <a href="#"><i class="fas fa-ship"></i> Trajet <span class="arrow"></span></a>
            </li>
            <ul class="sub-menu collapse" id="trajet">
                <li><a href="#">Ajouter</a></li>
                <li><a href="#">Supprimer</a></li>
                <li><a href="#">Editer</a></li>
            </ul>

            <li>
                <a href="#">
                    <i class="fas fa-clock"></i> Planification
                </a>
            </li>

            <li>
                <a href="#">
                    <i class="fas fa-cog"></i> Paramètres
                </a>
            </li>

        </ul>
    </div>
</div>
